...
class HomeController extends Controller {
	...
	public function index() {
		if (Auth::user()->user_type == "D") {
		  ...
		  // Fetch user locations with latitude and longitude for the admin view map display
$userLocations = DB::table('users')
    ->select('users.id',  'lat.value as latitude', 'lng.value as longitude')
    ->join('users_meta as lat', function($join) {
        $join->on('users.id', '=', 'lat.user_id')
             ->where('lat.key', '=', 'emsourcelat'); // Latitude key in users_meta
    })
    ->join('users_meta as lng', function($join) {
        $join->on('users.id', '=', 'lng.user_id')
             ->where('lng.key', '=', 'emsourcelong'); // Longitude key in users_meta
    })
    ->get();
    
    $userLocations = $userLocations->filter(function ($location) {
    return $location->latitude != "0" && $location->longitude != "0";
});
// Convert stdClass objects into a simpler array format for logging
$cleanedLocations = $userLocations->map(function ($location) {
    return [
        'id' => $location->id,
        ...
        'longitude' => $location->longitude,
			...
			return view('home', $index ,['userLocations' => $userLocations]);
		}
	}
...
//         // Including driver address and coordinates
//         $index['address'] = Auth::user()->address; // Address from the User table
//         $index['emsourcelat'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelat')->pluck('meta_value')->first();
//         $index['emsourcelong'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelong')->pluck('meta_value')->first();
...
//                 $data['time'] += $b->getMeta('waiting_time');
//                 $times = explode(" ", $b->getMeta('driving_time'));
//                 // Process the times as before...
//             }
//         }

//         // Including customer address and coordinates
//         $data['address'] = Auth::user()->address; // Address from the User table
//         $data['emsourcelat'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelat')->pluck('meta_value')->first();
//         $data['emsourcelong'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelong')->pluck('meta_value')->first();
...
//         // Get vehicle details for admin or other users
//         $vehicle_ids = array(0);
//         if (Auth::user()->group_id == null || Auth::user()->user_type == "S") {
//             $index['vehicles'] = VehicleModel::all()->count();
//             $index['bookings'] = Bookings::all()->count();
...
//         // Including admin address and coordinates if needed
//         $index['address'] = Auth::user()->address; // Address from the User table
//         $index['emsourcelat'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelat')->pluck('meta_value')->first();
//         $index['emsourcelong'] = Usermeta::where('user_id', Auth::user()->id)->where('meta_key', 'emsourcelong')->pluck('meta_value')->first();
