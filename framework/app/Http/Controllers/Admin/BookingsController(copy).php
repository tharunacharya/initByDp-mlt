...
// 			} elseif (Auth::user()->group_id == null || Auth::user()->user_type == "S" || Auth::user()->user_type == "O") {
...
//     try {
//         // Fetch the booking
//         $booking = Bookings::findOrFail($id); // Automatically throws 404 if not found
...
//         \Log::info('Available Drivers: ', $drivers->toArray());
...
//         \Log::info('Available Vehicles: ', $vehicles->toArray());

//         // Prepare data for the view
//         $index['drivers'] = $drivers;
//         $index['vehicles'] = $vehicles;
//         $index['data'] = $booking;
//         //  $index['customer_name'] = $booking->customer->name ?? 'N/A'; // Accessing customer name directly
...
//         // Decode the customer_id JSON array
//         $customerIds = json_decode($booking->customer_id, true);

//         // Fetch customer names based on the decoded IDs
//         $customers = User::whereIn('id', $customerIds)->pluck('name', 'id');

//         // Prepare data for the view
//         $index['drivers'] = $drivers;
//         $index['vehicles'] = $vehicles;
//         $index['data'] = $booking;
//         $index['customers'] = $customers; // Array of customer names
//         $index['udfs'] = unserialize($booking->getMeta('udf'));
...
public function edit($id) {
    try {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($customerIds)) {
            // If valid JSON, fetch customer names based on the decoded IDs
            $customers = User::whereIn('id', $customerIds)->pluck('name', 'id');
        } else {
            // If not valid JSON, treat as a single customer ID
            $customers = User::where('id', $booking->customer_id)->pluck('name', 'id');
        }
...
//       exit;
	...
	public function booking_notification($id)
	{
		...
		// $notificationData['key'] = "upcoming_ride_notification";
		// $notificationData['message'] = 'New Ride has been Assigned to you. from store';
		// \Log::info('Message added', $notificationData);
		...
		// 	// 'source_address' => "testing source",
		// 	// 'dest_address' => "testing dest",
		...
		// ];
		// \Log::info('Ride info added', $rideInfo);
		// $customerIds = json_decode($booking->customer_id, true);
		// Decode the customer_id (assuming it's stored as JSON string)
		// $customerIds = json_decode($booking->customer_id, true);
		// Check if the decoded result is an array and fetch the customer details


		// if (is_array($customerIds) && count($customerIds) > 0) {
		// 	// Fetch customer name using the first customer ID (or loop if needed)
		// 	$customer = User::find($customerIds[0]); // Assuming you're working with the first customer ID in the array
		// 	if ($customer) {
		// 		$userName = $customer->name ?? 'N/A';
		// 		$mobno = $customer->getMeta('mobno') ?? 'N/A';
		// 		$profilePic = $customer->getMeta('profile_pic') ?? 'N/A';

		// 		\Log::info('Customer Details', ['user_name' => $userName, 'mobno' => $mobno, 'profile_pic' => $profilePic]);
		// 	} else {
		// 		// Log an error if the user isn't found
		// 		// \Log::error('Customer not found', ['customer_id' => $customerIds[0]]);
		// 		$userName = 'N/A';
		// 		$mobno = 'N/A';
		// 		$profilePic = 'N/A';
		// 	}
		// } else {
		// 	\Log::error('Invalid customer_id format', ['customer_id' => $booking->customer_id]);
		// 	$userName = 'N/A';
		// 	$mobno = 'N/A';
		// 	$profilePic = 'N/A';
		// }
		...
		// ];
		// Log the user details array
		// \Log::info('User Details', ['user_details' => $userDetails]);
		...
		// Find the driver
		$driver = User::find($booking->driver_id);
		// 		\Log::info('Driver FCM ID: ' . $driver->getMeta('fcm_id'));
// \Log::info('Driver Availability: ' . $driver->getMeta('is_available'));
		...
		if (true) {
			...
			$deviceToken = 'fH3w3WpxR-y2Dmey43weky:APA91bFhNBJfslYpIJbZK_qKLGWRrihWixlDwGRiE-SpwYDWnxgDKHShJiuw6iFttzIznhdnhljmbGauFOgqIKBvmHQkoGX5SvJnQ3y5jpk9jW6FTTWvpAo';
			// Add custom data for FCM notification
			// $customData = [
			// 	'key' => $notificationData['key'],

			// 	'rideinfo' => json_encode($rideInfo), // Convert to JSON string

			// 	'user_details' => json_encode($userDetails), // Convert to JSON string
			// 	'data' => "hardcoded data just for testing",
			// ];
		...
		if (is_array($customerIds) && count($customerIds) > 0) {
			...
			if ($customer) {
				...
				$customerFCM = 'f0MImIXjSx6E8B-c4iT0aH:APA91bER2pXEFPR7AWKSWbhc206hjfMhcpFCIML-MPP_YtvC5RDjNtzAewIr6DH8nyuNV7x0EY7floHS6whnBmtpyJ4yDgCsZxbAMyMKDBJBEM-FLSQJzjI';
			}
		}
		...
		if ($customerFCM) {
			$customerTitle =
				...
				$customerResult = $firebaseService->sendNotificationWithCustomData($customerFCM, $title, $customerBody, $customData);
