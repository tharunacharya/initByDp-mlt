	...
	public function user_registration(Request $request) {
		...
		if (count($errors) > 0 || ($exist != null && $exist->deleted_at == null)) {
			...
			if ($exist == null) {
				...
				$id = User::create(['name' => $request->get('user_name'), 'email' => $request->get('emailid'), 'password' => bcrypt($request->get('password')), 'user_type' => 'C', 'api_token' => str_random(60),
				...
				$user->user_type = "C";
				$user->address = $request->get('address'); // Update address here
...
//     $booking->cancellation_reason = $request->input('reason', 'User requested cancellation');
//     $booking->save();
...
//             // Delete income record if booking status is not active
//             // if ($booking->status != 1) {
...
//             // Send cancellation emails (uncomment if needed)
...
//             return response()->json(['status' => 'error', 'message' => 'An error occurred while cancelling the booking.'], 500);
//         }
//     }
public function cancelBooking(Request $request)
{
    ...
    try {
        // Find the booking by cancel_id
        $booking = Bookings::find($request->cancel_id);

        // Check if the booking is already cancelled
        if ($booking->ride_status === 'Cancelled') {
            return response()->json(['status' => 'error', 'message' => 'This booking has already been cancelled.'], 400);
        }

        // Proceed with the cancellation if it's not already cancelled
        $booking->cancellation = 1;
        $booking->ride_status = "Cancelled";
        $booking->reason = $request->reason;
        $booking->save();

        // Optionally, delete income record (commented for now)
        // if ($booking->status != 1) {
        //     IncomeModel::where('income_id', $request->cancel_id)
        //               ->where('income_cat', 1)
        //               ->delete();
        ...
        // Optionally, send cancellation emails (commented for now)
        // if (Hyvikk::email_msg('email') == 1) {
        //     if ($booking->customer && $booking->driver) {
        //         Mail::to($booking->customer->email)->send(new BookingCancelled($booking, $booking->customer->name));
        //         Mail::to($booking->driver->email)->send(new BookingCancelled($booking, $booking->driver->name));
