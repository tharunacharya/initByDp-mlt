...
Route::get('/test-database', function (Database $database) {
    $reference = $database->getReference('test-data');
    $reference->set(['message' => 'Hello, Firebase!']); // Write data

    $data = $reference->getValue(); // Read data

    if (empty($data)) {
        return response()->json(['status' => 'error', 'message' => "Didn't get the message"], 404);
    }
...
Route::namespace ('Api')->middleware(['throttle'])->group(function () {
	...
	Route::post('/driver-registration', 'UsersApi@driver_registration');// driver regs
	
	

	Route::post('/user-login', 'UsersApi@user_login'); //user without social media connected
...
Route::namespace ('Api')->middleware(['throttle', 'auth:api'])->group(function () {
	...
	Route::post('/cancel-booking', 'UsersApi@cancelBooking');
	...
	Route::post('/assign-driver', 'DriversApi@driver_assign');// assig by admin req
