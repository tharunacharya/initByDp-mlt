...
<body>
    ...
    <script>
        ...
        function initMap() {
            ...
            const driversRef = database.ref("locations/Driver");
            driversRef.on("value", function(snapshot) {
                const drivers = snapshot.val();
                if (drivers) {
                    for (const driverId in drivers) {
                        const driver = drivers[driverId].location;
                        const lat = driver.latitude;
                        const lng = driver.longitude;

                        // Create a red marker for each driver with the label "D"
                        new google.maps.Marker({
                            position: { lat: lat, lng: lng },
                            map: map,
                            title: "Driver " + driverId,
                            label: "D",  // Label "D" for drivers
                            icon: {
                                path: google.maps.SymbolPath.CIRCLE,
                                fillColor: "red",  // Red color for drivers
                                fillOpacity: 1,
                                scale: 6,
                                strokeColor: "white",
                                strokeWeight: 2
            ...
            // // Fetch Rider locations from Firebase
            // const ridersRef = database.ref("locations/riders");
            // ridersRef.on("value", function(snapshot) {
            //     const riders = snapshot.val();
            //     if (riders) {
            //         for (const riderId in riders) {
            //             const rider = riders[riderId].location;
            //             const lat = rider.latitude;
            //             const lng = rider.longitude;

            //             // Create a blue marker for each rider with the label "R"
            //             new google.maps.Marker({
            //                 position: { lat: lat, lng: lng },
            //                 map: map,
            //                 title: "Rider " + riderId,
            //                 label: "R",  // Label "R" for riders
            //                 icon: {
            //                     path: google.maps.SymbolPath.CIRCLE,
            //                     fillColor: "blue",  // Blue color for riders
            //                     fillOpacity: 1,
            //                     scale: 6,
            //                     strokeColor: "white",
            //                     strokeWeight: 2
