...
<!-- Add this where you want the map on your dashboard -->
...
<!-- Add JavaScript for Google Maps and Live Tracking -->
<script>
    let map;
    let markers = {};

    function initMap() {
        const defaultLocation = { lat: 12.9716, lng: 77.5946 }; // Example: Bangalore
        map = new google.maps.Map(document.getElementById("map"), {
            center: defaultLocation,
            zoom: 12,
        });

        // Fetch and update markers every 5 seconds
        fetchVehicleDriverLocations();
        setInterval(fetchVehicleDriverLocations, 5000); // Update every 5 seconds
    }

    function fetchVehicleDriverLocations() {
        fetch('/api/locations') // Adjust to your actual endpoint
            ...
            .then(data => {
                data.forEach(location => {
                    ...
                    const markerKey = `${vehicle_id}-${driver_id}-${user_id}`; // Unique key for each marker

                    // Update existing marker or create a new one
                    if (markers[markerKey]) {
                        markers[markerKey].setPosition(new google.maps.LatLng(latitude, longitude));
                    } else {
                        markers[markerKey] = new google.maps.Marker({
                            position: { lat: parseFloat(latitude), lng: parseFloat(longitude) },
                            map,
                            title: `${type === 'driver' ? 'Driver' : 'Customer'}: ${name}`,
                            icon: type === 'driver' ? 'path_to_driver_icon.png' : 'path_to_customer_icon.png'
                        });
                    }
                });
            })
