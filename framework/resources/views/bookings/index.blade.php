...
//         ],
        ...
        render: function(data, type, row) {
            // Check if pickup data is JSON
            try {
                var pickupData = JSON.parse(data);
                if (pickupData && typeof pickupData === "object") {
                    // Assume pickupData has 'date' and 'time' fields after decoding JSON
                    return pickupData.date + ' ' + pickupData.time;
                }
            } catch (e) {
                // If not JSON, return data as is
                return data;
            }
        }
    },
